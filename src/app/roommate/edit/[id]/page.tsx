...
import { Loader2, Camera, MapPin, X, ChevronLeft, Save, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { uploadFileResumable } from '@/lib/tus-upload';
import Image from 'next/image';
import { AREA_LANDMARKS } from '@/lib/constants';

export default function EditRoommateAd() {
    ...
    const fetchListing = async () => {
        ...
        if (error || !data) {
            ...
            setFormData({
                ...
                rent: data.rent_per_person.toString(),
    ...
    try {
        const { error } = await supabase
            ...
            .update({
                ...
                status: 'active' // Ensure active on edit
            })
            .eq('id', id);
  ...
  if (fetching) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-blue-600"/></div>;
    ...
    <div className="min-h-screen bg-white px-6 py-6 pb-24">
        <header className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-black text-gray-900">Edit Room</h1>
            </div>
            <button onClick={handleDelete} className="p-2 text-red-500 bg-red-50 rounded-full">
                <Trash2 size={20} />
            </button>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            >
                {loading ? <Loader2 className="animate-spin" /> : <><Save size={20} /> Update Ad</>}
            </button>
        </form>
    </div>
  );
